"""metadata_time Agent (pair 型) - C10 真实实现。

modified_at 5 分钟滑窗跨投标人聚集 + created_at 精确相等双子信号。
"""
...
from app.services.detect.agents.metadata_impl.config import load_time_config
...
_DIMENSION = "metadata_time"
_ALGORITHM = "metadata_time_v1"
...
async def preflight(ctx: AgentContext) -> PreflightResult:
    ...
    a_ok = await bidder_has_metadata(ctx.session, ctx.bidder_a.id, "modified")
    b_ok = await bidder_has_metadata(ctx.session, ctx.bidder_b.id, "modified")
    if a_ok and b_ok:
...
def _build_summary(dim_result, is_ironclad: bool) -> str:
    ...
    if not hits:
        return "未发现时间聚集或创建时间碰撞"
    dims = sorted({h.get("dimension", "?") for h in hits})
    prefix = "[铁证] " if is_ironclad else ""
    return f"{prefix}时间维度命中:{','.join(dims)}(共 {len(hits)} 条)"


@register_agent("metadata_time", "pair", preflight)
async def run(ctx: AgentContext) -> AgentRunResult:
    ...
    if not cfg.enabled:
        evidence = {
            ...
            "enabled": False,
            "score": None,
            "reason": "METADATA_TIME_ENABLED=false",
    ...
    try:
        ...
        return AgentRunResult(
            score=0.0,
            summary=f"metadata_time 执行失败:{type(e).__name__}",
            evidence_json=evidence,
        )
