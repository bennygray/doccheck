...
每个候选 sheet 返回一项:
- sheet_name: 原样
- sheet_role: "main" | "breakdown" | "summary" — 见下文「sheet_role 判定」
...
- **排除**:以下类型的 sheet 即使出现在文件里也**不要**放进 sheets_config:
  ...
  - 空表、只有表头无数据的表
  - 纯文字说明(备注、条款)
- 同 xlsx 多个价格表 sheet → 各自独立返回,column_mapping 可不同

**sheet_role 判定**(关键字段,影响下游总价计算):
- `main`:主报价表 / 独立分项;参与总价 SUM
- `breakdown`:某 main 的明细分解(同钱多视角);不参与 SUM
- `summary`:整 sheet 汇总;不参与 SUM(罕见)

规则:
- 多 sheet 合价 SUM 近似相等且某表项目数明显少 → 少的为 main,多的为 breakdown
- 各 sheet 合价 SUM 不同(独立分项,如土建/安装/电气)→ 全部 main
- 不确定时倾向 main(漏分类比误标 breakdown 安全)
