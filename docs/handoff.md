...
| 项 | 值 |
|---|---|
| 当前里程碑 | **M4 完成 + V1 全量验收 + admin-llm-config + fix-mac-packed-zip-parsing + honest-detection-results + harden-async-infra + agent-skipped-error-guard + llm-classifier-observability + test-infra-followup-wave2 + fix-admin-users-page-flaky-test + config-llm-timeout-default(CH-3)+ parser-accuracy-fixes(CH-1)+ detect-template-exclusion(CH-2)+ fix-llm-timeout-default-followup + fix-section-similarity-spawn-loop + fix-bug-triple-and-direction-high + fix-dimension-i18n-stale-text(2026-04-27 归档)** |
| 当前 change | `fix-dimension-i18n-stale-text` **归档完成**(2026-04-27)。前序 `fix-bug-triple-and-direction-high` 引入 2 维但 4 处 `DIMENSION_LABELS` 同源 map 只改 1 处(ReportPage.tsx),漏 3 处(DimensionDetailPage / ComparePage / DetectProgressIndicator);加 2 处 stale "11 个维度" 字面量(ReportPage:545 + DetectProgressIndicator:163)。最简纯前端 i18n + copy 修补,**4 文件 6 字面量**,无后端、无 spec、无契约改动。L1 frontend tsc 0 errors + Vitest 114/114 ✅;L2 跳过(零后端);**L3 Claude_in_Chrome manual 4 截图凭证**(`e2e/artifacts/fix-dimension-i18n-stale-text-2026-04-27/`):检测进度面板"超过最高限价"+"投标总额完全相等"中文 grid / 报告"13 个维度的得分雷达" / 维度明细 price_overshoot 行中文 / 对比总览。**Follow-up 登记 backlog**:抽 `frontend/src/utils/dimensionLabels.ts` 共享 util(本次维持 4 处独立避 refactor 风险) + AI 综合研判 LLM 文本中 `price_overshoot` 英文 key(LLM 自生成,不在 UI map 控制范围,需改 prompt template) |
