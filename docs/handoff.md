...
| 项 | 值 |
|---|---|
| 当前里程碑 | **M4 完成 + V1 全量验收 + ... + fix-dimension-i18n-stale-text + fix-unit-price-orphan-fallback + fix-multi-sheet-price-double-count(2026-04-28 归档)** |
| 当前 change | `fix-multi-sheet-price-double-count` **归档完成**(2026-04-28)。**根因**:招标方下发的监理标 xlsx 模板含 3 sheet(报价表 1 行=主报价 / 管理人员单价表 5 行=主报价的人月明细分解 / 人员进场计划)+ Sheet2 row9 是合计行 — 三视角描述同一笔钱 X,但旧 PriceItem 扁平模型把所有 sheet 都加和 → SUM=3X 虚高 → price_overshoot 假阳性铁证。**5 阶段深度调查**(现状采样 + 代码追溯 + 6 次历史 patch 复盘 + 多源对照 + 第一性原理审)定位为**架构层抽象错误**,不是边缘 bug。**修法 A+B+F 三层防线**:A 汇总行 deterministic skip(`fill_price.py` item_name 含汇总关键字 + unit_price 为空 → skip);B LLM sheet_role 显式分类(`prompts.py` 加 `main/breakdown/summary` 三值枚举 + parser 解析校验);F 数值关系兜底校验(`sheet_role_validator.py`:两 sheet SUM 1% 容差内相等且 LLM 都标 main → 行少为 main 行多为 breakdown);下游 SUM 过滤(`aggregate_bidder_totals` + `compare_price` 共用 `is_main_sheet_clause` JSONB EXISTS,仅 SUM sheet_role='main')。alembic 0013 backfill 老数据 sheet_role='main' 兼容。L1 **1223 passed**(原 1182 + 新 41:8 汇总行 + 5 sheet_role parse + 12 validator + 2 compare_price 过滤 + 5 anomaly extractor fixture 修补 + 8 fill_price 既有 + 1 schema);L2 **291 passed**(原 286 + 新 3 e2e fixture:监理标 / 工程量清单 / LLM 错兜底 + 2 fixture 修);L3 真实 walkthrough(3 zip,id=3295)实测**总报价 = 456k/486k/674.8k 真实价**(原 1368k/1458k/2024.4k 虚高 3x 已消除),price_overshoot/price_total_match 都 score=0 不再误触发。**关键经验**:历次 patch 都把"多 sheet 在 SUM 维度可加"当作前提,只在边缘处理(跨文件 / 备注行 / 序号列 / 模板簇),从来没人挑战这个核心假设;fix-unit-price 作者已经意识到主表+子表重复求和但 scope 局限于跨文件维度。凭证 `e2e/artifacts/fix-multi-sheet-price-double-count-2026-04-28/`(README 含修前 vs 修后对照表 + 5 截图 + 2 JSON dump) |
...
| 最新 commit | `ea447c2` 归档 change: fix-dimension-i18n-stale-text(M4 follow-up,4 文件 6 字面量) — fix-unit-price-orphan-fallback 待 archive 自动 commit |
