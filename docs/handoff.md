...
| 先前 change | `fix-admin-users-page-flaky-test` 归档完成。`test-infra-followup-wave2` 遗留的前端 AdminUsersPage `创建用户成功` 全量跑 flaky;`AdminUsersPage.test.tsx` L100 `userEvent.setup({ delay: null })` + L81 test 第 3 参数 15000;`pipeline-error-handling` spec +1 Requirement "前端交互测试 timing 契约"。前端 L1 **114/114 绿连续 3 次稳定**;scope 锁死只改 1 站点;零产品行为变化 |
| 先前 change | `test-infra-followup-wave2` 归档完成。5 项测试/诊断基础设施 follow-up 合 1 change:**Item 1(🔴 real bug)** `alembic/env.py:27` 加 `disable_existing_loggers=False`(治 L2 全绿);Item 2 engine except AST 元测试;Item 3 `run_isolated` finally future-proof Py 3.14+;Item 4 main.py lifespan 顶部 `setLevel(INFO)` 让 uvicorn 级联 app logger;Item 6 DimensionRow 补 text_sim degraded case;`pipeline-error-handling` spec +1 Req "测试基础设施鲁棒性契约"。L1 1020/1020 + L2 **281/281 绿**;零产品行为变化 |
| 先前 change | `llm-classifier-observability` 归档完成。N3 LLM 大文档精度退化 explore 后的观测性 change:`role_classifier.py` 3 条 info 日志补齐 3 决策路径的诊断信号 + `_looks_mojibake` heuristic 识别乱码文件名 + 1 ADDED Requirement 进 parser-pipeline spec;双采样脚本 B 方案 2 轮证明 N3 原始症状不再复现(根因 H2a 已被 `fix-mac-packed-zip-parsing` 消除)。L1 1011/1011 + L2 280 passed,零回归。|
| 最新 commit | parser-accuracy-fixes 归档(CH-2 归档后会刷新) |
