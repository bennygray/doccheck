...
| 项 | 值 |
|---|---|
| 当前里程碑 | **M4 完成 + V1 全量验收 + ... + text-sim-exact-match-bypass + detect-tender-baseline §7+§8 完成(2026-04-30,待归档)** |
| 当前 change | `detect-tender-baseline` (M5)。**§1–§8 全部完成 + L1/L2/L3 三层测试全绿 + L3 7 张截图 + JSON 凭证落档**,**待归档** (9.2 handoff 更新本次,9.3 archive commit 待用户授权)。**§7** 前端基线 UI 全套(commit `e75e227`):VITE_TENDER_BASELINE_ENABLED=false 默认灰度兜底;5 新组件(TenderUploadCard / BaselineStatusBadge / StartDetectPreCheckDialog / RerunAfterTenderDialog / StaleBaselineBadge)+ ProjectDetailPage / StartDetectButton / ReportPage / DimensionRow / TextComparePage / Compare/Meta/PriceComparePage 接入;types/index.ts 加 TenderDocument / BaselineSource / BaselineStatus + PC 级 / 段级 baseline_matched 可选字段;services/api.ts 加 listTenders / uploadTender / deleteTender;frontend vitest 30 文件 145 测试 0 失败 + tsc 0 errors。**§8.0** backend 透传补齐(本次 commit):schemas/compare.py + routes/compare.py 把 PC.evidence_json.samples baseline_matched/source 透传到 TextMatch;schemas/report.py + routes/reports.py 把 PC.evidence_json baseline_source 聚合到 ReportDimension + ReportResponse 顶级(取所有维度最强 source:tender > consensus > metadata_cluster > none),否则前端 ReportPage Badge 永远 'none' / DimensionRow Tag 永远不出。**§8.7** 真 LLM 双 zip run:project 4194(L1-synthetic-tender-baseline-demo)+ tender + 3 合成 vendor(98.4% 段命中 tender);v=2 25 succeeded + 2 skipped,**报告级 baseline_source='tender'** + 维度级 price_consistency / price_anomaly baseline_source='tender' + UI Badge L1 蓝。实测成本 < ¥1(远低于预估 ¥3-5,因合成 zip 高度重复 → exact_match path 短路)。**§8.6** 7 张截图凭证(5 设计要求 + 2 bonus L3 + DimensionRow Tag)归档 `e2e/artifacts/detect-tender-baseline-2026-04-30/l3-full-demo/screenshots/`。**§8.4 (双栏对比段灰底) UI 注入说明**:真 LLM 把高相似段判为 plagiarism label,§3 design D8 决定 baseline 旁路只对 exact_match 生效 → 真路径 baseline_matched=false。截图 ④ 通过 `tmp_inject_baseline_for_ui_demo.py` 直接 SQL 标 PC 512 前 2 sample baseline_matched=true,UI 演示用;功能正确性由 backend §3 e2e 5 case 全覆盖。**回归**:backend e2e 全 323(原 322)+ unit 1370 + frontend vitest 145 全绿,0 回归。**已识别但本 change 不修**:① TenderDocument soft-delete 后 md5 dedup 不重置(用 v2 zip 绕过);② 真 LLM exact_match label 命中率取决于 LLM 判断,本 demo plagiarism > exact_match。 |
| 先前 change | `text-sim-exact-match-bypass` **归档完成**(2026-04-29)。**根因**(reviewer 多轮纠正):**双锁** — (i) 段长 < `MIN_PARAGRAPH_CHARS=50` 触发 `_merge_short_paragraphs` 的 buf 累积合并,A、C 合并段尾不同 → cosine 远低于 1.0;(ii) 真模板段 28 对 cosine ≥ 0.9925 占满 `MAX_PAIRS_TO_LLM=30` 上限,注入段被截断 → LLM 不见 → evidence 不收录。**第一性原理纠正**:TF-IDF 中 token 完全一致 cosine 必然 = 1.0(IDF 是全局缩放对夹角不变),问题不在 IDF 而在 segmenter 合并 + cap 截断双重作用。**修法**:(a) `tfidf.py` 入口前加 `_hash_pairs` 段级 sha1 精确匹配旁路(归一化 NFKC + `\s+` 合并 + strip),hash 命中段直接 sim=1.0 / label='exact_match' / 不送 LLM / cosine 候选集排除已命中对;(b) `MAX_PAIRS_TO_LLM` 30 → 60;(c) `aggregator._WEIGHTS` 加 `exact_match=1.0`;(d) `models.ParaPair` 加可选 `match_kind` 字段;(e) `llm_judge.call_llm_judge` 加 token 溢出 truncate;(f) `compare.py` left/right_text_map 改 `dict[str, list[int]]` 笛卡尔积修早存 UI bug;(g) `segmenter.SegmentResult` 加 `merged_anchor_paragraph_index` 字段。**L3 真 LLM 实测** `e2e/artifacts/text-sim-exact-match-bypass-2026-04-29/`。**演进路径备忘**(v2 ngram/MinHash/shingle)见下方 §1.1。 |
| 先前 change | `fix-multi-sheet-price-double-count` **归档完成**(2026-04-28)。**根因**:招标方下发的监理标 xlsx 模板含 3 sheet(报价表 1 行=主报价 / 管理人员单价表 5 行=主报价的人月明细分解 / 人员进场计划)+ Sheet2 row9 是合计行 — 三视角描述同一笔钱 X,但旧 PriceItem 扁平模型把所有 sheet 都加和 → SUM=3X 虚高 → price_overshoot 假阳性铁证。**修法 A+B+F 三层防线** + alembic 0013 backfill。L1 **1223** + L2 **291** + L3 真实 walkthrough(id=3295)实测真实价(原 1368k/1458k/2024.4k 虚高 3x 消除)。凭证 `e2e/artifacts/fix-multi-sheet-price-double-count-2026-04-28/` |
| 先前 change | `fix-unit-price-orphan-fallback` **归档完成**(2026-04-28)。**症状**:服务器生产 LLM(DeepSeek `deepseek-v4-flash`)对监理报价表(首段含"综合单价")100% 误判 `file_role='unit_price'`,而 `unit_price` 在下游报价回填管线 0 消费 → bidder 卡 `identified` 终态、price_items=0、UI 空白。**修法**:`run_pipeline.py` 两个 helper(`_find_pricing_xlsx` / `_find_all_pricing_xlsx`)按 role 优先级 fallback —— 先 pricing,空时退 unit_price;**单 bidder 单类不变量**(elif 三分支互斥)保护下游 `aggregate_bidder_totals(SUM)` 不被"主表+子单价表混算"污染(避免新增的 `price_overshoot` / `price_total_match` 铁证级 detector 误报升 high)。**3 处不变量保护**:① extractor.py 的 SUM 不分桶 / ② price_overshoot 直接吃 SUM(超限即铁证) / ③ price_total_match 同上。**反 over-engineering**:不抽通用 helper(reviewer 提的 DRY 建议被否)、不加 DB 字段、不动 detector。**A-F 6 维全域调查**(乱码线物理验证排除 / unit_price 设计史确认孤儿 / file_role+price_items 消费方矩阵 / LLM 高敏调用点全清单);proposal/design 含 7 行下游消费方矩阵 + 5 Decision + 3 Future Work。L1 **12/12 绿**(4 Scenario × 2 helper + 对称性 3 fixture parametrize + sheet mismatch regression);L2 **2/2 绿**(2 家全 unit_price → 全 priced + aggregate 返 2 条;1 家 pricing+unit_price 共存仅 pricing 进 + price_overshoot.run 集成断言 score=0 防混算误报);**L3 真实 UI walkthrough**(`e2e/artifacts/fix-unit-price-orphan-fallback-2026-04-28/`):本地火山 ark 走 happy path 验证主路径无回归(本地不复现服务器 DeepSeek 误判 — README 含范围说明 + Chrome MCP file_upload 拒绝改 API 的操作变通透明记录) |
| 先前 change | `fix-dimension-i18n-stale-text` **归档完成**(2026-04-27)。前序 `fix-bug-triple-and-direction-high` 引入 2 维但 4 处 `DIMENSION_LABELS` 同源 map 只改 1 处(ReportPage.tsx),漏 3 处(DimensionDetailPage / ComparePage / DetectProgressIndicator);加 2 处 stale "11 个维度" 字面量(ReportPage:545 + DetectProgressIndicator:163)。最简纯前端 i18n + copy 修补,**4 文件 6 字面量**,无后端、无 spec、无契约改动。L1 frontend tsc 0 errors + Vitest 114/114 ✅;L2 跳过(零后端);**L3 Claude_in_Chrome manual 4 截图凭证**(`e2e/artifacts/fix-dimension-i18n-stale-text-2026-04-27/`)。**Follow-up 登记 backlog**:抽 `frontend/src/utils/dimensionLabels.ts` 共享 util(本次维持 4 处独立避 refactor 风险) + AI 综合研判 LLM 文本中 `price_overshoot` 英文 key |
| 先前 change | `fix-bug-triple-and-direction-high` **归档完成**(2026-04-27,commit 47f731f)。修 3 个用户报告 bug + 9 个同根因对称性盲点("半成品契约":input 字段定义但 detect 不消费 / SSE 协议 publish 漏字段 / 算法维度对齐错位)。**4 轮 reviewer 35+ HIGH 全部吸收** — v1→v4 三次"补丁式修订"被找出 24 HIGH(scorer 数学不支持 ironclad / amount_pattern 维度错配 / lift hook 漏 prop chain / watchdog 阈值 < heartbeat 等),v4 self-review 后转向 happy-path 数据流第一性倒推。代码:**A 后端 SSE 协议**(judge.py:489 publish project_status_changed MUST 早于 report_ready / engine.py:135 crash 路径 publish + status=ready / scanner.py:174 回滚也 publish);**B 前端 hook lift**(useDetectProgress 从 HeroDetectArea 子组件提到 ProjectDetailPage 父 + props chain;hook 初值 "draft"→null + Tag `??` 区分;watchdog 35s 业务事件 lastBizEventAt 不算 heartbeat;DetectEventType union 加 project_status_changed/error;useParseProgress 同款 dead listener 修复);**C/D 2 个新 global Agent**(price_total_match 任意两家 total 完全相等 → has_iron_evidence;price_overshoot 任一超限 → has_iron_evidence,max_price=NULL/≤0 skip);**E 权重重平衡**(error_consistency 0.12→0.10 / style 0.08→0.07 / image_reuse 0.05→0.02 释放 0.06 给 2 新维度各 0.03,和=1.00);**F UI**(DIMENSION_LABELS+SHORT 加 2 维 + AdminRulesPage label "报价天花板"→"异常低价偏离" 修语义错位 + DimensionRow 既有 is_ironclad Tag 自动适配);**G 11→13 维巡检**(test_detect_judge / test_detect_registry / test_rules_mapper / test_export_generator / test_reports_api / e2e task_count 11→13/25→27 + judge_llm.py prompt + 前端 6 处 11 维注释 + EXPECTED_AGENT_COUNT 11→13 + admin-rules spec dimensions 10→12)。spec delta 3 capability:project-status-sync MODIFY 既有 SSE Requirement 加 3 scenarios(detect 完成 + crash + scanner) / detect-framework ADD 2 Requirement(price_overshoot + price_total_match) / admin-rules MODIFY Get rules config API(10→12)。L1 **1182/8 skipped**(原 1166 + 新 16);L2 **286/2 skipped**(11→13 task_count 修订 + judge_llm e2e fixture 加 2 新 OA);前端 typecheck ✅;**L3 降级 manual + 凭证**(`e2e/artifacts/fix-bug-triple-and-direction-high-2026-04-27/`);Manual 真实 e2e 留 follow-up(L1+L2 已锁根因) |
...
| 最新 commit | `e75e227` apply: detect-tender-baseline §7 前端基线 UI(D12 ⑦) — §8 后端 patch + L3 凭证 + handoff 更新由本次 commit 包含 |
...
## 1.1 演进路径备忘(text_similarity v2,本次 change archive 时合入)

**触发**: `text-sim-exact-match-bypass` proposal 显式承认 hash 旁路是过渡方案,reviewer Round 2 第 4 条要求标注 ngram/MinHash/shingle 演进路径,避免 6 个月后变 legacy。

**v1 救不了的场景**:

| 场景 | v1 失败原因 | v2 候选解 |
|---|---|---|
| 47 字 < 50 字短段抄袭 | `_merge_short_paragraphs` buf 累积合并把 47 字段和相邻段拼到一起,A、C 合并段尾不同 → cosine 0.81 / hash 不命中 | (a) **n-gram shingling**: 段拆 4-5 字 shingle + Jaccard;短段也能比对 (b) **段级 MinHash**: 在 raw body 段做 MinHash, 容许局部修改 |
| 改 1 字的近似抄袭 | hash 100% 字符相同才命中, 改 1 字 hash 全变 | **edit distance** 或 **MinHash** 容许 1-3% 编辑距离 |
| 大模板化文档 cap=80 触发 LLM 300s timeout | 单次 LLM 调用上下文太长 | (a) **分批 LLM**: 80 段拆 4 批×20 段并行 (b) **本地 BERT 分类**: 替代 LLM judge 长期降本 |

**v2 候选 change 草稿**:
- `text-sim-shingle-fuzzy-match`: n-gram + Jaccard,救短段抄袭 + 改字近似抄袭
- `text-sim-llm-batched-judge`: cap 拆批并行,救大文档 timeout
- `text-sim-ironclad-length-tunable`: 50 字门槛改可配置(产品/客户场景不同)
- (评估)`text-sim-bert-judge`: 本地 BERT 替代 LLM 长期降本
...
- **i18n util follow-up**(2026-04-27 from `fix-dimension-i18n-stale-text`):4 处 `DIMENSION_LABELS` map 仍是独立副本(DimensionDetailPage / ReportPage 长 + 短 / ComparePage / DetectProgressIndicator),抽 `frontend/src/utils/dimensionLabels.ts` 共享 util。本次"最简 change"风险考量未做。再加 1 维时若仍重复改 4 处则触发独立 change。
- **AI 研判英文 key follow-up**(同上):LLM 综合研判文案输出含 `price_overshoot` 等英文 key,不在 UI label map 范围,需改 `judge_llm.py` prompt template 或 OutputAnalysis 后处理本地化。
- **R12 follow-up**:image_reuse 未来引入 has_iron_evidence 写入 → 决定是否纳入剔除白名单
- **R5 follow-up**:style 部分覆盖 N-gram 精细化(本 change 简化为 "全覆盖才剔除")
- **跨模型 LLM 一致性 follow-up**(2026-04-28 from `fix-unit-price-orphan-fallback`):本地火山 ark 与生产 DeepSeek `deepseek-v4-flash` 行为不一致,本次 unit_price 误判仅是冰山一角;F 维度调查列出 7 个 LLM 调用点其中 3 个高敏(role 分类 / price_rule / error_l5 铁证)。后续需立独立 change:① 本地补 DeepSeek 回归通道(.env LLM 切换 + L2 双跑) ② 高敏调用点加 confidence 阈值或人工二次审核(尤其 error_l5 铁证认定 — 一个 true 直接强制升 high)③ 跨模型一致性测试套件
- **`unit_price` 角色定位决议 follow-up**(同上):本 change 用 fallback 兜底解决眼前 silent failure,但 `unit_price` 仍是"标记但 0 下游消费"的孤儿。后续需产品决策:要么真正赋予下游处理(如对比子单价表本身的相似度),要么从 9 角色枚举里彻底删除 — 当前"标记但孤儿"是技术债
- **`aggregate_bidder_totals` 分桶 follow-up**(同上):extractor.py:40 SUM 不分桶仍是隐性炸弹;本 change 用"单 bidder 单类不变量"绕过它,根本解决需按 sheet 或 source 分桶
- **L3 fallback 路径真 UI 验证 follow-up**(同上):本地火山 ark 不触发 unit_price 误判 → 本次 L3 仅验证主路径无回归;真实 fallback UI 表现需用 DeepSeek 切换或 SQL 手工注入复现,留待生产环境 deploy 后用户走一次端到端
