...
| 项 | 值 |
|---|---|
| 当前里程碑 | **M4 完成 + V1 全量验收 + ... + fix-multi-sheet-price-double-count + text-sim-exact-match-bypass(2026-04-29 归档)** |
| 当前 change | `text-sim-exact-match-bypass` **归档完成**(2026-04-29)。**根因**(reviewer 多轮纠正):**双锁** — (i) 段长 < `MIN_PARAGRAPH_CHARS=50` 触发 `_merge_short_paragraphs` 的 buf 累积合并,A、C 合并段尾不同 → cosine 远低于 1.0;(ii) 真模板段 28 对 cosine ≥ 0.9925 占满 `MAX_PAIRS_TO_LLM=30` 上限,注入段被截断 → LLM 不见 → evidence 不收录。**第一性原理纠正**:TF-IDF 中 token 完全一致 cosine 必然 = 1.0(IDF 是全局缩放对夹角不变),问题不在 IDF 而在 segmenter 合并 + cap 截断双重作用。**修法**:(a) `tfidf.py` 入口前加 `_hash_pairs` 段级 sha1 精确匹配旁路(归一化 NFKC + `\s+` 合并 + strip),hash 命中段直接 sim=1.0 / label='exact_match' / 不送 LLM / cosine 候选集排除已命中对;(b) `MAX_PAIRS_TO_LLM` 30 → 60(L3 实测 80 在大文档下 cap 触发 300s timeout 折中到 60);(c) `aggregator._WEIGHTS` 加 `exact_match=1.0`,`compute_is_ironclad` 改自包含完整规则 — pairs_plagiarism≥3 / 占比≥50% / 含 ≥1 段归一化长度≥50 的 exact_match → True;degraded 永远 False(spec D8);(d) `models.ParaPair` 加可选 `match_kind` 字段;(e) `llm_judge.call_llm_judge` 加 `_estimate_prompt_tokens` + 24K 溢出按 sim 降序 truncate,evidence.degraded_reason='token_overflow';(f) **顺手修早存 UI bug**:`compare.py` 的 `left_text_map`/`right_text_map` 用 dict 推导式同字符串后覆盖前,改成 `dict[str, list[int]]` 笛卡尔积派生 match — 这才让 NEW A 注入段 [11] + [682] 两处都高亮(本 bug 在 v1 时即存在,本 change 一并修);(g) `segmenter.SegmentResult` 加 `merged_anchor_paragraph_index` 字段,evidence_json.samples.a_idx/b_idx 从合并段索引转回 raw `DocumentText.paragraph_index` 体系,前端 `TextComparePage.leftMatchMap.get(p.paragraph_index)` 才能命中。**spec delta**:detect-framework MOD 2 Req(双轨→三轨;evidence_json schema 加 pairs_exact_match)+ ADD 2 Req(段级 hash 精确匹配旁路 + exact_match label 优先级与 ironclad 长度门槛 50)。**3 轮 reviewer**(proposal R1+R2:7+3 必修反馈全吸收,specs R1:6 收口意见全吸收)— "IDF 稀释"误判被推翻 / 新增 "score drift / label 优先级 / token 溢出兜底" 三条 proposal 必修 / "scenario 名 hash 优先于 LLM 改成 label 不被 LLM 二次覆写" / 删 "覆盖原规则" 措辞自包含。L1 **56 全过**(text_sim_impl 原 38 baseline + 新 17 case:hash 旁路三段命中 / NFKC + \s+ + strip 等价性 / ironclad 49/50 字门槛 / 降级 + cosine 候选空仍触发 ironclad / cap 不影响算分公式 / token truncate / pairs_exact_match 字段);L1 detect 全套 **185 全过**;L2 **3 新+2 baseline 全过**(hash 命中段进 evidence top + 不送 LLM / 旧版 evidence 兼容 / version 递增);**L3 真 LLM 实测** `e2e/artifacts/text-sim-exact-match-bypass-2026-04-29/`(用客户演示 zip `tmp_repro2/同样句式加在第一段/`):project=3297 / A=3722 / C=3723 / v=5 / score=86.65 / 高风险 / 3 条铁证;DOM 高亮颜色 JSON 实测 165 字"检查材料保管"3/3 全亮(rgba(194,124,14,0.38))/ 131 字"必须坚持榜样"1/2 / 47 字"啊啊啊"0/2(已知短段限制留 v2)。**5 次 detection version 演进**(v1 cap=80 timeout / v2 cap=60 hash 在 merged succeeded 但 a_idx 体系不映射 / v3-v4 hash 在 raw 段 timeout / v5 hash 在 merged + segmenter anchor 转 raw paragraph_index succeeded)。**proposal 显式承认过渡方案** — 改 1 字近似抄袭 / 47 字 < 50 短段稀释 留 v2 ngram/MinHash/shingle 路径(见下方"演进路径备忘") |
...
| 最新 commit | `<archive-pending>` 归档 change: text-sim-exact-match-bypass(M4 follow-up,客户演示痛点修)— 待 archive 自动 commit 后回填 hash |
