...
- **THEN** 响应 422(拒绝越界),或服务端自动截断至 100(实现方可二选一,L2 测试锁定选择)
...
`projects` 表 MUST 包含以下字段:`id UUID PK`(或 BIGSERIAL,实现方选择)、`name VARCHAR(100) NOT NULL`、`bid_code VARCHAR(50) NULL`、`max_price NUMERIC(18,2) NULL`、`description VARCHAR(500) NULL`、`status VARCHAR(32) NOT NULL DEFAULT 'draft'`、`risk_level VARCHAR(16) NULL`、`owner_id` FK→ `users.id` NOT NULL、`created_at TIMESTAMPTZ NOT NULL DEFAULT now()`、`updated_at TIMESTAMPTZ NOT NULL DEFAULT now()`、`deleted_at TIMESTAMPTZ NULL`。MUST 建立联合索引 `(owner_id, deleted_at, created_at DESC)` 支持 reviewer 列表主路径。
